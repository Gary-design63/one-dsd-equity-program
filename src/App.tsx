...
// PRD 10 Core Modules + supporting pages
import Dashboard from "@/pages/Dashboard";
import ConsultationsPage from "@/pages/ConsultationsPage";
import KnowledgeBasePage from "@/pages/KnowledgeBasePage";
import TrainingPage from "@/pages/TrainingPage";
import DeiaCalendarPage from "@/pages/DeiaCalendarPage";
...
// Existing pages retained as sub-modules
...
// PRD 30/70 navigation model
const NAV_ITEMS: NavItem[] = [
  // 70% Consultant-facing
  { label: "Dashboard", path: "/", group: "Consultant (70%)", shortLabel: "Dashboard" },
  { label: "Approval Queue", path: "/approval-queue", group: "Consultant (70%)", shortLabel: "Queue" },
  { label: "KPIs", path: "/kpis", group: "Consultant (70%)", shortLabel: "KPIs" },
  { label: "Equity Assist", path: "/equity-assist", group: "Consultant (70%)", shortLabel: "Assist" },
  { label: "Institutional Memory", path: "/institutional-memory", group: "Consultant (70%)", shortLabel: "Memory" },
  // 30% Staff-facing
  { label: "Consultations", path: "/consultations", group: "Staff (30%)", shortLabel: "Consult" },
  { label: "Resource Library", path: "/knowledge", group: "Staff (30%)", shortLabel: "Library" },
  { label: "Learning", path: "/training", group: "Staff (30%)", shortLabel: "Learning" },
  ...
  { label: "Metrics", path: "/metrics", group: "System", shortLabel: "Metrics" },
  { label: "Settings", path: "/settings", group: "System", shortLabel: "Settings" },
];

const NAV_GROUPS = ["Consultant (70%)", "Staff (30%)", "System"];

function getPageSummary(pathname: string) {
  if (pathname.startsWith("/approval-queue")) {
    return { metric: "5 pending", label: "items awaiting review", icon: Bell };
  }
  if (pathname.startsWith("/consultations")) {
    return { metric: "4 active", label: "open consultations", icon: Bell };
  }
  if (pathname === "/") {
    return { metric: "30/70", label: "operating model active", icon: LayoutGrid };
  }
  return { metric: "Live", label: "platform ready", icon: Bell };
}
      ...
      >
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {!isCollapsed && (
              <div className="min-w-0">
                ...
                <div className="truncate text-base font-semibold text-white">People &amp; Culture</div>
              </div>
            )}
          </div>
        ...
        <div className={cn("border-b border-white/10", isCollapsed ? "px-3 py-3" : "px-4 py-4") }>
          <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-3">
            <div className={cn("flex items-center gap-3", isCollapsed && "justify-center") }>
              ...
              {!isCollapsed && (
                <div className="min-w-0">
                  <div className="text-xs uppercase tracking-[0.18em] text-white/45">Owner</div>
                  <div className="text-sm font-medium text-white">Equity &amp; Inclusion Consultant</div>
                </div>
              )}
            </div>
          </div>
        ...
        <div className={cn("border-t border-white/10", isCollapsed ? "px-3 py-3" : "px-4 py-4")}>
          <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-3 text-white/70">
            {!isCollapsed ? (
              <>
                <div className="text-xs uppercase tracking-[0.18em] text-white/40">Program</div>
                <div className="mt-2 flex items-center justify-between text-sm">
                  <span>Model</span>
                  <span className="font-medium text-white">30/70</span>
                </div>
        ...
        <div className="flex h-16 items-center gap-3 px-4 md:px-6">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-xs uppercase tracking-[0.16em] text-muted-foreground">
              ...
              <span>{currentNav?.group ?? "Consultant (70%)"}</span>
            </div>
            <div className="mt-0.5 flex items-center gap-3">
              <h1 className="truncate text-lg font-semibold text-foreground">{currentNav?.label || "Dashboard"}</h1>
          ...
          <div className={cn("ml-auto flex items-center gap-2", isSidebarCollapsed ? "xl:gap-3" : "xl:gap-2") }>
            <label className="header-search hidden md:flex">
              ...
              <input
                type="text"
                placeholder="Search documents, consultations, resources"
            ...
            <Button
              ...
              data-testid="button-upload-shell"
            >
            ...
            <input
              ref={uploadRef}
              type="file"
              accept=".json,audio/*,video/*,image/*,.pdf,.doc,.docx,.txt,.csv,.xlsx,.pptx,.md,.html"
              className="hidden"
              onChange={(e) => {
        ...
        <div className="flex min-w-0 flex-1 flex-col lg:h-screen">
          ...
          <main id="main-content" className="flex-1 overflow-auto" style={{ fontSize: `${zoom}%` }} tabIndex={-1}>
            <Routes>
              ...
              <Route path="/institutional-memory" element={<InstitutionalMemoryPage />} />
              {/* Staff (30%) */}
          ...
          <footer className="border-t border-border/70 bg-background/90 px-6 py-3 text-xs text-muted-foreground">
            {PROGRAM_NAME} · Minnesota Department of Human Services · Disability Services Division · 2026
          </footer>
        </div>
      </div>
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="rounded-3xl border border-border/70 bg-card px-8 py-7 text-center shadow-[0_24px_80px_rgba(15,23,42,0.10)]">
          ...
          <p className="text-sm font-medium text-foreground">Loading {PROGRAM_NAME}</p>
          <p className="mt-1 text-xs text-muted-foreground">Preparing the equity operations platform.</p>
        </div>
      </div>
    );
  }
